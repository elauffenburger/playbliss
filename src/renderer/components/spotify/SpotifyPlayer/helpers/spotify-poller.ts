...
import { MusicSource, Track, SpotifyTrack, PlaylistTrack } from "../../../../models";
...
// The max size of the pendingStateQueue
...
export class SpotifyPoller {
  ...
  async checkPlaybackState() {
    ...
    if (this.shouldAbortPlaybackStatusPoll(playerState)) {
      return;
    }
    ...
    const { trackProgressMs } = this.updateTrackProgress(state.spotifyTrack, state.trackContext);
    ...
    if (playerState.track && playerState.track.track.id != state.track.id) {
    ...
    if (this.trackHasEnded(playerWasPlaying, originalTrack, state.track, trackProgressMs)) {
      this.player.notifyTrackEnded({ track: state.track });
    }
  }

  private shouldAbortPlaybackStatusPoll(playerState: PlayerState): boolean {
  ...
  private trackHasEnded(
    ...
    const trackHasEnded =
      ...
      (progressDelta <= SONG_ENDED_THRESHOLD_MS || trackProgressMs <= SONG_ENDED_THRESHOLD_MS);
  ...
  statesAgree(): boolean {
    ...
    return this.pendingStateQueue.reduce((statesAgree, pendingState) => {
      ...
      const tracksAgree =
        noTrack ||
        (!!state.track && !!pendingState.track && state.track.id == pendingState.track.id);
